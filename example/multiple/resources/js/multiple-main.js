;(function(win, doc, $) {
  ...
  const page = {
    ...
    // 第二个列表
    list2: [],
    isXhrGetList2Complete: true,
    xhrGetList2(pageIndex) {
      const self = this;
      const url = `https://easy-mock.com/mock/5ba05fc2dadd12214f1f47c3/api/online-test/home/ajax/newspage?pageindex=${pageIndex}&pagesize=${PAGE_SIZE}`;

      this.isXhrGetList2Complete = false;
      request(url, 'GET').then(res => {
        const { status } = res;
        if (status) {
          const { count, list } = res;
          ...
          self.isXhrGetList2Complete = true;

          const oldList = JSON.parse(JSON.stringify(self.list2));
          const lists = oldList.concat(list);

          loadMore2.pageIndex = pageIndex;
          loadMore2.pageCount = count;
          const isCanLoadMore = loadMore2.isCanLoadMore();
          if (!isCanLoadMore) {
            this.hideLoadMore2();
          }
        
          this.renderList2(lists);
